...
///<summary>
/// Centralizes magic strings for REST API conventions.
///</summary>
module ApiLiterals =
    /// <summary>conventional key in the <see cref="RestApiMetadata.ClaimsSet" /> dictionary</summary>
    [<Literal>]
    let ClaimKeyForEndPointPrefix = "endpoint-prefix"
...
type ApiBase =
    ...
    static member fromConfiguration (input: IConfiguration) (name :string) =
        let key = input.GetValue $"{ApiLiterals.RestApiMetadata}:{name}:{nameof ApiBase}"
        match key with
...
type ClaimsSet =
    ...
    static member fromConfiguration (input: IConfiguration) (name :string) =
        let key = $"{ApiLiterals.RestApiMetadata}:{name}:{nameof ClaimsSet}"
